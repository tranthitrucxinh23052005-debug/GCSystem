...
// ─── Interfaces (Định nghĩa kiểu để hết lỗi Property not exist) ──────────────
interface Tier {
    nameEn: string;
    nameVi: string;
    min: number;
...
function getTier(pts: number): Tier {
    return TIERS.find(t => pts >= t.min && pts < t.max) || TIERS;
}
...
export default function GCSSimulatorPrototype() {
    // ─── States (Fix lỗi never[] và Implicit any) ────────────────────────────
    ...
    const [txType, setTxType] = useState<any>(TX_TYPES);
    ...
    // Quan trọng: Khai báo mảng any[] để không bị lỗi SetStateAction<never[]>
    ...
    const handleTransfer = () => {
        ...
        setTimeout(() => {
            ...
            setHistory(h => [{
                id: Date.now(), type: 'earn', label: txType.label.split(' '),
    ...
    return (
        <div style={{ maxWidth: 430, margin: '0 auto', minHeight: '100dvh', background: '#f8fafc', position: 'relative', display: 'flex', flexDirection: 'column' }}>
            {/* ── Toast notification ── */}
            {showToast && (
                <div style={{ position: 'fixed', top: 16, left: '50%', transform: 'translateX(-50%)', background: toastGcs >= 0 ? '#065f46' : '#7c3aed', color: '#fff', padding: '10px 20px', borderRadius: 40, fontWeight: 600, fontSize: 13, zIndex: 999, boxShadow: '0 8px 32px rgba(0,0,0,.2)', display: 'flex', alignItems: 'center', gap: 8, animation: 'slideDown .3s ease', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ background: 'linear-gradient(145deg, #064e3b, #065f46)', padding: '52px 20px 24px', color: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        ...
                        <p style={{ fontSize: 28, fontWeight: 800 }}>{gcsPoints.toLocaleString('vi-VN')} <span style={{ fontSize: 14, fontWeight: 500, opacity: .8, marginLeft: 4 }}>điểm</span></p>
                    </div>
                ...
                {nextTier && (
                    <div style={{ marginTop: 14 }}>
                        ...
                        <div style={{ background: 'rgba(255,255,255,.2)', borderRadius: 99, height: 6 }}>
                            <div style={{ width: `${Math.min(tierProgress, 100)}%`, height: '100%', background: '#6ee7b7', borderRadius: 99, transition: 'width .6s cubic-bezier(.34,1.56,.64,1)' }} />
                        </div>
                    </div>
                )}
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 80 }}>
                {activeTab === 'home' && (
                    <div style={{ padding: 16 }}>
                        <div style={{ background: timeCoef > 1 ? '#fef9c3' : '#f1f5f9', border: `1px solid ${timeCoef > 1 ? '#fde047' : '#e2e8f0'}`, borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontSize: 12 }}>
                        ...
                        <div style={{ background: '#fff', borderRadius: 20, padding: 20, boxShadow: '0 1px 8px rgba(0,0,0,.06)', marginBottom: 16 }}>
                            <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 14, color: '#1e293b' }}><Send size={14} style={{ verticalAlign: -2, marginRight: 6 }} /> Thực hiện thanh toán</p>
                            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: '1.5px solid #e2e8f0', fontSize: 18, fontWeight: 700, marginBottom: 12 }} />
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 16 }}>
                                {TX_TYPES.map(t => (
                                    <button key={t.value} onClick={() => setTxType(t)} style={{ padding: '8px 10px', borderRadius: 10, fontSize: 11, border: txType.value === t.value ? '1.5px solid #059669' : '1px solid #e2e8f0', background: txType.value === t.value ? '#f0fdf4' : '#f8fafc', color: txType.value === t.value ? '#059669' : '#64748b', fontWeight: 600 }}>{t.label}</button>
                                ))}
                            </div>
                            <button onClick={handleTransfer} disabled={processing} style={{ width: '100%', padding: 14, borderRadius: 14, background: '#059669', color: '#fff', fontWeight: 700, border: 'none' }}>
                                {processing ? 'Đang xử lý...' : 'Thanh toán & Tích GCS'}
                            </button>
                        </div>
                    </div>
                )}
                {/* (Các tab Market, History, Block giữ nguyên logic render) */}
            </div>

            {/* ── Bottom navigation ── */}
            <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, background: '#fff', borderTop: '1px solid #f1f5f9', display: 'flex', paddingBottom: 10 }}>
                {[
                    ...
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ flex: 1, padding: '10px 0', border: 'none', background: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
